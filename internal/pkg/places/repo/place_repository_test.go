...
func TestPlaceRepository_GetPlaces(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT p.id, p.name, p.imagePath, p.description, p.rating, p.numberOfReviews, p.address, p.phoneNumber, c.name AS city_name, ARRAY_AGG(ca.name) AS categories FROM place p JOIN city c ON p.cityId = c.id JOIN place_category pc ON p.id = pc.place_id JOIN category ca ON pc.category_id = ca.id GROUP BY p.id, c.name ORDER BY p.id LIMIT $1 OFFSET $2")).
		WithArgs(10, 0).
