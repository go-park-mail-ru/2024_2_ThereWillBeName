...
func (r *PlaceRepository) GetPlaces(ctx context.Context, limit, offset int) ([]models.GetPlace, error) {
	query := "SELECT p.id, p.name, p.image_path, p.description, p.rating, p.numberOfReviews, p.address, p.phone_number, c.name AS city_name, ARRAY_AGG(ca.name) AS categories FROM place p JOIN city c ON p city_id = c.id JOIN place_category pc ON p.id = pc.place_id JOIN category ca ON pc.category_id = ca.id GROUP BY p.id, c.name ORDER BY p.id LIMIT $1 OFFSET $2"
...
func (r *PlaceRepository) CreatePlace(ctx context.Context, place models.CreatePlace) error {
	query := "INSERT INTO place (name, image_path, description, rating, numberOfReviews, address, city_id, phone_number) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id"
	var id int
...
func (r *PlaceRepository) GetPlace(ctx context.Context, id uint) (models.GetPlace, error) {
	var place models.GetPlace
	query := "SELECT p.id, p.name, p.image_path, p.description, p.rating, p.numberOfReviews, p.address, p.phone_number, c.name AS city_name, ARRAY_AGG(ca.name) AS categories FROM place p JOIN city c ON p.city_id = c.id JOIN place_category pc ON p.id = pc.place_id JOIN category ca ON pc.category_id = ca.id WHERE p.id = $1 GROUP BY p.id, c.name ORDER BY p.id"
...
func (r *PlaceRepository) UpdatePlace(ctx context.Context, place models.UpdatePlace) error {
	query := "UPDATE place SET name = $1, image_path = $2, description = $3, rating = $4, numberOfReviews = $5, address = $6, phone_number = $7, updated_at = NOW() WHERE id=$8"
...
func (r *PlaceRepository) SearchPlaces(ctx context.Context, name string, limit, offset int) ([]models.GetPlace, error) {
	var places []models.GetPlace
	query := "SELECT p.id, p.name, p.image_path, p.description, p.rating, p.numberOfReviews, p.address, p.phone_number, c.name AS city_name, ARRAY_AGG(ca.name) AS categories FROM place p JOIN city c ON p.city_id = c.id JOIN place_category pc ON p.id = pc.place_id JOIN category ca ON pc.category_id = ca.id WHERE p.name LIKE '%' || $1 || '%' GROUP BY p.id, c.name ORDER BY p.id LIMIT $2 OFFSET $3"
